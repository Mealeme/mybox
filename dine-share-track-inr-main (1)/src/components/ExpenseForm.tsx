...
const ExpenseForm: React.FC<ExpenseFormProps> = ({ onExpenseAdded, expenseToEdit, onCancelEdit }) => {
  const [expenses, setExpenses, refreshExpenses] = useLocalStorage<Expense[]>('expenses', []);
  ...
  // Refresh expenses on mount to ensure we have the latest data
  useEffect(() => {
    refreshExpenses();
  }, [refreshExpenses]);
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    if (isEditing && currentExpenseId) {
      ...
      // Only update the local state if no callback is provided
      if (onExpenseAdded) {
